...
public class util {
	...
	public static int shellSortVueltas(int[] arr) {
		int contador = 0;
        int n = arr.length;
        for (int gap = n / 2; gap > 0; gap /= 2) {
            for (int i = gap; i < n; i++) {
	...
	static float numerosRepetidos (int[] arreglo){
	...
	static float cantidadRepetida (int[] arreglo){
		float calificacionMRepi = 0;
		ordenarQuicksort(arreglo, 0, arreglo.length-1);
		int contador = 1;
		for (int i = 1; i < arreglo.length; i++){
	...
	public static int ordenamientoBurbujaVueltas(int[] arreglo) {
		int contador=0;
        int n = arreglo.length;
        boolean intercambiado;
        for (int i = 0; i < n - 1; i++) {
